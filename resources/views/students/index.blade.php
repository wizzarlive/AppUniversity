...
<div class="p-0" x-data="{ openCreate: false, openEdit: false, student: {} }" x-cloak>
                    ...
                    @foreach ($students as $student)
                        <tr>
                            ...
                            <td class="px-3 py-4 whitespace-nowrap text-xs font-medium">
                                <button @click="openEdit = true; student = {{ $student }};"
    ...
    <div x-show="openCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div @click.away="openCreate = false" class="bg-white rounded-2xl shadow-xl sm:max-w-2xl w-full">
            ...
            <form action="{{ route('students.store') }}" method="POST" class="px-8 pb-8 space-y-5">
                @csrf
    ...
    <div x-show="openEdit" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div @click.away="openEdit = false" class="bg-white rounded-2xl shadow-xl sm:max-w-2xl w-full">
            ...
            <form :action="`/students/${student.id}`" method="POST" class="px-8 pb-8 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-black mb-1">DNI</label>
                        <input type="text" maxlength="8" name="dni" x-model="student.dni" required
                            class="w-full bg-[#EEEEEE] rounded-md py-3 px-4 text-sm text-gray-700 placeholder-gray-500 focus:ring-2 focus:ring-[#0B0641]">
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-sm font-bold text-black mb-1">Número Telefónico</label>
                    <input type="text" maxlength="9" name="phone" x-model="student.phone" required
                        class="w-full bg-[#EEEEEE] rounded-md py-3 px-4 text-sm text-gray-700 placeholder-gray-500 focus:ring-2 focus:ring-[#0B0641]">
                </div>

                <div>
                    <label class="block text-sm font-bold text-black mb-1">Curso</label>
                    <select name="course_id" x-model="student.course_id" required
                        class="w-full bg-[#EEEEEE] rounded-md py-3 px-4 text-sm text-gray-600 cursor-pointer focus:ring-2 focus:ring-[#0B0641]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-bold text-black mb-1">Estado</label>
                        <select name="status" x-model="student.status" required
                            class="w-full bg-[#EEEEEE] rounded-md py-3 px-4 text-sm text-gray-600 cursor-pointer focus:ring-2 focus:ring-[#0B0641]">
                            <option value="Activo">Activo</option>
                            <option value="Inactivo">Inactivo</option>
                        </select>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-black mb-1">Ciclo</label>
                        <select name="ciclo" x-model="student.ciclo" required
                            class="w-full bg-[#EEEEEE] rounded-md py-3 px-4 text-sm text-gray-600 cursor-pointer focus:ring-2 focus:ring-[#0B0641]">
